	...
	<div class="page-grid sobriety-grid">
		<SectionCard title="Today">
			...
			<div class="button-row">
				{#each sobrietyStatusActions as action (action.status)}
					<Button variant={action.variant === 'secondary' ? 'secondary' : undefined} onclick={() => markStatus(action.status)}>
						{action.label}
					</Button>
				{/each}
			</div>
		...
		<SectionCard title="Craving check-in">
			<Field label="Craving score">
				<input bind:value={page.cravingScore} aria-label="Craving score" type="number" min="0" max="5" />
			</Field>
