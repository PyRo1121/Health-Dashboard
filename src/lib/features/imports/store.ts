...
import type { ClinicalPatientIdentity, LocalPatientCandidate } from '$lib/features/integrations/identity/patient-match';
...
export async function createImportBatch(db: HealthDatabase, sourceType: ImportSourceType): Promise<ImportBatch> {
	const timestamp = nowIso();
...
export async function dedupeImportedEvents(
	...
	for (const event of events) {
		const existing = await db.healthEvents.where('sourceRecordId').equals(event.sourceRecordId ?? '').first();
...
export async function previewImport(
	...
	if (resolvedSourceType === 'smart-fhir-sandbox') {
		...
		const match = resolveClinicalPatientMatch(imported.patientIdentity, [ownerCandidate(input.ownerProfile)]);
