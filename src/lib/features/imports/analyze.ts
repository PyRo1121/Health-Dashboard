	...
	return {
		...
		summary: records.length
			? buildReadySummary({
					...
					detail: 'The import center detected Apple Health XML and can stage these records before commit.',
	...
	return {
		...
		summary: entries.length
			? buildReadySummary({
					...
					detail: 'The import center detected a Day One export and can stage these entries before commit.',
	...
	return {
		...
		summary: warnings
			...
			: buildReadySummary({
					...
					detail: 'The import center detected a compatible iPhone bundle and can stage it before commit.',
					itemCount: normalized.bundle.records.length,
					itemLabel: 'records',
					metricTypes
				})
...
export function analyzeImportPayload(rawText: string, filename?: string): ImportPayloadAnalysis | null {
...
export function describeImportPayload(rawText: string, filename?: string): ImportPayloadSummary {
	return analyzeImportPayload(rawText, filename)?.summary ?? {
		filename,
