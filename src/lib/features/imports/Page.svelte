<script lang="ts">
	...
	const { config: importSourceConfig, options: importSourceOptions, labels: sourceLabels } =
		importSourceCatalog;
	...
	let showHelperLinks = $derived(Boolean(currentHelperCopy?.links && allowsHelperLinks(payloadSummary)));
...
{:else}
	<div class="page-grid imports-grid">
		<ImportComposerCard
			...
			{selectedFileName}
			{payloadSummary}
			healthkitManifest={healthkitManifest}
			{sourceLabels}
			{currentHelperCopy}
			{showHelperLinks}
			{currentSampleBundle}
