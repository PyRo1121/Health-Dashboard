...
const PROTEIN_KEYWORDS = ['chicken', 'turkey', 'beef', 'salmon', 'tuna', 'egg', 'tofu', 'beans', 'lentil', 'yogurt'];
const FIBER_KEYWORDS = ['beans', 'lentil', 'chickpea', 'oat', 'broccoli', 'spinach', 'kale', 'quinoa', 'brown rice', 'vegetable'];
...
function scoreFood(item: FoodCatalogItem, context: NutritionRecommendationContext): NutritionRecommendation {
	...
	if (calories >= minCalories && calories <= maxCalories) {
		...
		score -= Math.min(20, Math.abs((calories < minCalories ? minCalories - calories : calories - maxCalories) / 25));
	}
...
function scoreRecipe(item: RecipeCatalogItem, context: NutritionRecommendationContext): NutritionRecommendation {
