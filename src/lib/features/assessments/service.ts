...
async function getStoredAssessment(
	...
	return db.assessmentResults.where('localDay').equals(localDay).and((entry) => entry.instrument === instrument).first();
}
...
function buildAssessmentRecord(input: {
	...
	return {
		...updateRecordMeta(input.existing, `assessment:${input.instrument}:${input.localDay}`, timestamp),
