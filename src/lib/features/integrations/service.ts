...
function latestTimestamp(current: string | undefined, event: HealthEvent): string | undefined {
	const candidate =
		asString(event.payload?.capturedAt) ?? event.sourceTimestamp ?? event.updatedAt ?? event.createdAt;
...
export async function summarizeNativeCompanionEvents(db: HealthDatabase): Promise<NativeCompanionSummary> {
	const events = (await db.healthEvents.toArray()).filter((event) => event.sourceType === 'native-companion');
