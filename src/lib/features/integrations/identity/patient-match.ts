...
export function resolveClinicalPatientMatch(
	...
	const existingMapping = existingMappings.find((mapping) => mapping.sourceScopeKey === sourceScopeKey);
	...
	if (demographicMatches.length > 1) {
		return {
			...
			requiresExplicitConfirmation: true,
			candidateMatches: demographicMatches,
			reason: 'More than one local person matches this clinical record. Block import until resolved.'
		};
	}
