...
function buildClinicalEvent(input: {
	...
	return {
		...
		sourceRecordId: buildSourceRecordId(input.patientIdentity.sourcePatientId, input.resourceType, input.resourceId),
...
function buildObservationEvent(
	...
	const value =
		typeof quantityValue === 'number'
			? quantityValue
			: asString(quantityValue) ?? asString(resource.valueString) ?? asString(resource.status) ?? 'recorded';
...
export function normalizeSmartFhirBundle(bundle: SmartFhirBundle): SmartFhirImportResult {
	...
	const patientEntry = bundle.entry.find((entry) => asRecord(entry.resource)?.resourceType === 'Patient');
