...
function buildManualHealthRow(event: HealthEvent): HealthEventRow {
  ...
  if (event.eventType === 'symptom') {
    ...
    const severity =
      typeof payload.severity === 'number' ? `Severity ${payload.severity}/5` : undefined;
    return {
      id: event.id,
      title: symptom,
      badge: severity,
  ...
  if (event.eventType === 'anxiety-episode') {
    ...
    const intensity =
      typeof payload.intensity === 'number' ? `Intensity ${payload.intensity}/5` : undefined;
    return {
      id: event.id,
  ...
  if (event.eventType === 'sleep-note') {
    ...
    const restfulness =
      typeof payload.restfulness === 'number' ? `Restfulness ${payload.restfulness}/5` : undefined;
    return {
      id: event.id,
      title: 'Sleep note',
      badge: restfulness,
...
export function createHealthEventRows(events: HealthEvent[]): HealthEventRow[] {
  return events.map((event) =>
    event.eventType === 'sleep-duration'
      ? buildSleepDurationRow(event)
      : buildManualHealthRow(event)
  );
}
