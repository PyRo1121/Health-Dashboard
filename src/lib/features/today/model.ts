import type { DailyCheckinInput, TodayPlannedWorkout, TodaySnapshot } from '$lib/features/today/service';
...
export function createTodayNutritionGuidance(snapshot: TodaySnapshot | null): string[] {
	...
	if (!guidance.length) {
		if (protein < 30) {
			guidance.push('Protein is still low so far. A 20g+ meal would change the day more than another snack.');
		} else if (fiber < 10) {
			guidance.push('Fiber is still low so far. Oats, beans, berries, or greens would give the day more shape.');
		} else {
			guidance.push('Nutrition is on a steady enough path. Keep the next meal simple and repeatable.');
		}
	}
