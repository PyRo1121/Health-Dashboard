...
export function normalizeUsdaFoodDetail(
	...
	const protein = firstMatchingAmount(food.foodNutrients, [{ number: '1003' }, { name: 'Protein' }]);
...
async function readJson<T>(url: string, init: RequestInit, fetchImpl: typeof fetch = fetch): Promise<T> {
