...
class SqliteTable<T extends BaseRecord> implements HealthDbTable<T> {
	...
	constructor(
		...
		const columns = ['id', ...this.indexedColumns.filter((column) => column !== 'id'), 'record_json'];
		...
		return {
			equals: (value: T[Key]): HealthDbQuery<T> =>
				new ArrayQuery(async () =>
					...
					}) as Array<{ record_json: string }>).map((row) => this.deserializeRow(row))
				)
		};
	}
}
...
function createTableMap(database: Database): Pick<
	HealthDatabase,
	| 'dailyRecords'
	| 'journalEntries'
	| 'foodEntries'
...
> {
	return {
		dailyRecords: new SqliteTable(database, 'dailyRecords', parseIndexedColumns(SCHEMA_STORES.dailyRecords)),
		journalEntries: new SqliteTable(database, 'journalEntries', parseIndexedColumns(SCHEMA_STORES.journalEntries)),
		foodEntries: new SqliteTable(database, 'foodEntries', parseIndexedColumns(SCHEMA_STORES.foodEntries)),
		...
		planSlots: new SqliteTable(
			database,
			'planSlots',
			parseIndexedColumns(SCHEMA_STORES.planSlots)
		),
		...
		favoriteMeals: new SqliteTable(database, 'favoriteMeals', parseIndexedColumns(SCHEMA_STORES.favoriteMeals)),
		healthEvents: new SqliteTable(database, 'healthEvents', parseIndexedColumns(SCHEMA_STORES.healthEvents)),
		...
		importBatches: new SqliteTable(database, 'importBatches', parseIndexedColumns(SCHEMA_STORES.importBatches)),
...
export async function withServerHealthDb<T>(run: (db: HealthDatabase) => Promise<T> | T): Promise<T> {
	if (isPlaywrightMode()) {
