...
test('smart sandbox import requires an owner profile and lands in timeline once configured', async ({ page }) => {
	const importsPage = new ImportsPage(page);
	await importsPage.goto();
...
test('integrations page routes users into the import center', async ({ page }) => {
	await page.goto('/integrations');
	await expect(page.getByRole('table', { name: 'Clinical connector capability matrix' })).toBeVisible();
...
test('served shortcut kit endpoints are downloadable', async ({ page }) => {
	const template = await page.request.get('/downloads/ios-shortcuts/healthkit-companion-template.json');
