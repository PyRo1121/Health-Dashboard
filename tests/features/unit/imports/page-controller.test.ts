...
describe('imports page controller', () => {
	it('creates the default page state and updates source/drag state', () => {
		...
		expect(setImportsPageSourceType(initial, 'day-one-json').intake.sourceType).toBe('day-one-json');
	...
	it('loads sample and file payloads into intake state', async () => {
		...
		const fileState = await loadImportsFilePayload(createImportsPageState(), file, () => READY_SUMMARY);
