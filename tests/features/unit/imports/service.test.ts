...
<HealthData>
  <Record type="HKQuantityTypeIdentifierStepCount" sourceName="iPhone" unit="count" value="4321" />
	...
	it('stages Apple Health import preview and keeps duplicate replay idempotent', async () => {
		const db = getDb();
		const first = await previewImport(db, { sourceType: 'apple-health-xml', rawText: APPLE_HEALTH_XML });
		...
		const second = await previewImport(db, { sourceType: 'apple-health-xml', rawText: APPLE_HEALTH_XML });
	...
	it('stages HealthKit companion bundle previews and dedupes replay', async () => {
		const db = getDb();
		const first = await previewImport(db, { sourceType: 'healthkit-companion', rawText: HEALTHKIT_BUNDLE_JSON });
		...
		const second = await previewImport(db, { sourceType: 'healthkit-companion', rawText: HEALTHKIT_BUNDLE_JSON });
