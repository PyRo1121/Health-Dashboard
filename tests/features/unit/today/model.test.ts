...
describe('today model', () => {
	it('hydrates the form, rows, and payload from a snapshot', () => {
		...
		expect(createTodayFormFromSnapshot(snapshot)).toMatchObject({ mood: '4', freeformNote: 'Steady start.' });
		...
		expect(createDailyCheckinPayload(snapshot.date, createTodayFormFromSnapshot(snapshot))).toMatchObject({
			date: '2026-04-02',
			mood: 4,
			focus: 5
		});
