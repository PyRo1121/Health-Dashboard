...
describe('assessments route', () => {
  ...
  it('loads and saves assessments through the server route', async () => {
    ...
    ).toEqual(expect.objectContaining({ saveNotice: 'Progress saved.' }));
    expect(
      await (
        await POST({
          request: new Request('http://health.test/api/assessments', {
    ...
    ).toEqual(expect.objectContaining({ saveNotice: 'Assessment saved.' }));
  });
  ...
  it('returns 400 for invalid assessments payloads', async () => {
    ...
    const response = await POST({
      request: new Request('http://health.test/api/assessments', {
        method: 'POST',
        body: JSON.stringify({ action: 'saveProgress' }),
      }),
    } as Parameters<typeof POST>[0]);
