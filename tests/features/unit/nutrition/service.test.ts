...
describe('nutrition service', () => {
	const getDb = useTestHealthDb('nutrition-service');
	...
	it('reuses a recurring meal into a new day', async () => {
		...
		const favorite = await saveFavoriteMeal(db, {
			name: 'Quick oats',
			mealType: 'breakfast',
			items: [{ name: 'Oatmeal with berries', calories: 320, protein: 12, fiber: 8, carbs: 52, fat: 7 }]
		});
		...
		const reused = await reuseRecurringMeal(db, { favoriteMealId: favorite.id, localDay: '2026-04-02' });
