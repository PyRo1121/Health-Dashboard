...
describe('nutrition route', () => {
  ...
  async function importRoute(overrides: {
    ...
    vi.doMock('$lib/server/nutrition/service', () => ({
      ...
      saveNutritionMealServer:
        overrides.saveNutritionMealServer ??
        vi.fn(async () => ({ ...pageState, saveNotice: 'Meal saved.' })),
      planNutritionMealServer:
        overrides.planNutritionMealServer ??
        vi.fn(async () => ({ ...pageState, saveNotice: 'Planned next meal saved.' })),
      saveNutritionRecurringMealServer:
        overrides.saveNutritionRecurringMealServer ??
        vi.fn(async () => ({ ...pageState, saveNotice: 'Recurring meal saved.' })),
      saveNutritionCatalogItemServer:
        overrides.saveNutritionCatalogItemServer ??
        vi.fn(async () => ({ ...pageState, saveNotice: 'Saved to custom food catalog.' })),
      clearNutritionPlannedMealServer:
        overrides.clearNutritionPlannedMealServer ??
        vi.fn(async () => ({ ...pageState, saveNotice: 'Planned meal cleared.' })),
      reuseNutritionMealServer:
        overrides.reuseNutritionMealServer ??
        vi.fn(async () => ({ ...pageState, saveNotice: 'Recurring meal reused.' })),
  ...
  it('loads nutrition page state through the server route', async () => {
    ...
    const {
      pageState,
      route: { POST },
    } = await importRoute({ loadNutritionPageServer });
    ...
    expect(await response.json()).toEqual(
      expect.objectContaining({ localDay: '2026-04-04', loading: false })
    );
  ...
  it('dispatches nutrition write actions through the server route', async () => {
    ...
    ).toEqual(expect.objectContaining({ saveNotice: 'Saved to custom food catalog.' }));
    expect(
      await (
        await POST({
          request: new Request('http://health.test/api/nutrition', {
    ...
    ).toEqual(expect.objectContaining({ saveNotice: 'Planned next meal saved.' }));
    expect(
      await (
        await POST({
          request: new Request('http://health.test/api/nutrition', {
    ...
    ).toEqual(expect.objectContaining({ saveNotice: 'Recurring meal saved.' }));
    expect(
      await (
        await POST({
          request: new Request('http://health.test/api/nutrition', {
            method: 'POST',
            body: JSON.stringify({ action: 'clearPlannedMeal', state: pageState }),
          }),
        } as Parameters<typeof POST>[0])
      ).json()
    ).toEqual(expect.objectContaining({ saveNotice: 'Planned meal cleared.' }));
    expect(
      await (
        await POST({
          request: new Request('http://health.test/api/nutrition', {
            ...
            body: JSON.stringify({
              action: 'reuseMeal',
              state: pageState,
              favoriteMealId: 'favorite-1',
            }),
          }),
        } as Parameters<typeof POST>[0])
      ).json()
    ).toEqual(expect.objectContaining({ saveNotice: 'Recurring meal reused.' }));
  });
  ...
  it('returns 400 for invalid nutrition action payloads', async () => {
    ...
    const response = await POST({
      request: new Request('http://health.test/api/nutrition', {
        method: 'POST',
        body: JSON.stringify({ action: 'planMeal', localDay: '2026-04-04' }),
      }),
    } as Parameters<typeof POST>[0]);
