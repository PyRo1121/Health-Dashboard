...
describe('planning controller', () => {
	...
	it('loads, saves plan slots, reorders them, and updates groceries', async () => {
		...
		expect(state.saveNotice).toBe('Plan slot saved.');
		...
		expect(state.saveNotice).toBe('Plan slot removed.');
		expect(state.slots).toHaveLength(1);
	});
	...
	it('guards missing linked items and saves workout templates', async () => {
		...
		expect(state.saveNotice).toBe('Choose a recipe before adding a meal slot.');
		...
		expect(state.saveNotice).toBe('Choose a workout template before adding a workout slot.');
	...
	it('surfaces stale linked-item errors for recipe, food, and workout slots', async () => {
		...
		expect(state.saveNotice).toBe('That recipe no longer exists. Choose another before adding it to the week.');
		...
		expect(state.saveNotice).toBe('That saved food no longer exists. Choose another before adding it to the week.');
