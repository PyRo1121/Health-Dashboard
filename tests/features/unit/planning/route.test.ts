...
describe('planning route', () => {
  ...
  it('loads planning page state through the server route', async () => {
    const loadPlanningPageServer = vi.fn(async () => ({
      loading: false,
      localDay: '2026-04-07',
      slots: [],
    }));
    ...
    expect(await response.json()).toEqual(
      expect.objectContaining({ localDay: '2026-04-07', loading: false })
    );
  ...
  it('dispatches planning write actions through the server route', async () => {
    ...
    const savePlanningSlotPageServer = vi.fn(async () => ({
      ...state,
      planNotice: 'Plan slot saved.',
    }));
    const saveWorkoutTemplatePageServer = vi.fn(async () => ({
      ...state,
      workoutTemplateNotice: 'Workout template saved.',
    }));
    const markPlanningSlotStatusPageServer = vi.fn(async () => ({
      ...state,
      planNotice: 'Plan slot marked done.',
    }));
    const movePlanningSlotPageServer = vi.fn(async () => ({
      ...state,
      planNotice: 'Plan slot moved up.',
    }));
    const deletePlanningSlotPageServer = vi.fn(async () => ({
      ...state,
      planNotice: 'Plan slot removed.',
    }));
    const togglePlanningGroceryStatePageServer = vi.fn(async () => ({
      ...state,
      groceryNotice: 'Grocery item updated.',
    }));
    const addManualPlanningGroceryItemPageServer = vi.fn(async () => ({
      ...state,
      groceryNotice: 'Manual grocery item added.',
    }));
    const removeManualPlanningGroceryItemPageServer = vi.fn(async () => ({
      ...state,
    ...
    const saveSlotResponse = await POST({
      request: new Request('http://health.test/api/plan', {
        method: 'POST',
        body: JSON.stringify({ action: 'saveSlot', state }),
      }),
    } as Parameters<typeof POST>[0]);
    expect(await saveSlotResponse.json()).toEqual(
      expect.objectContaining({ planNotice: 'Plan slot saved.' })
    );
    ...
    expect(await templateResponse.json()).toEqual(
      expect.objectContaining({ workoutTemplateNotice: 'Workout template saved.' })
    );
    ...
    const markResponse = await POST({
      request: new Request('http://health.test/api/plan', {
        method: 'POST',
        body: JSON.stringify({ action: 'markSlotStatus', state, slotId: 'slot-1', status: 'done' }),
      }),
    } as Parameters<typeof POST>[0]);
    expect(await markResponse.json()).toEqual(
      expect.objectContaining({ planNotice: 'Plan slot marked done.' })
    );
    ...
    const moveResponse = await POST({
      request: new Request('http://health.test/api/plan', {
        method: 'POST',
        body: JSON.stringify({ action: 'moveSlot', state, slotId: 'slot-1', direction: 'up' }),
      }),
    } as Parameters<typeof POST>[0]);
    expect(await moveResponse.json()).toEqual(
      expect.objectContaining({ planNotice: 'Plan slot moved up.' })
    );
    ...
    const deleteResponse = await POST({
      request: new Request('http://health.test/api/plan', {
        method: 'POST',
        body: JSON.stringify({ action: 'deleteSlot', state, slotId: 'slot-1' }),
      }),
    } as Parameters<typeof POST>[0]);
    expect(await deleteResponse.json()).toEqual(
      expect.objectContaining({ planNotice: 'Plan slot removed.' })
    );
    ...
    const toggleResponse = await POST({
      request: new Request('http://health.test/api/plan', {
        ...
        body: JSON.stringify({
          ...
          state,
          itemId: 'grocery-1',
          patch: { checked: true, excluded: false, onHand: false },
        }),
      }),
    } as Parameters<typeof POST>[0]);
    expect(await toggleResponse.json()).toEqual(
      expect.objectContaining({ groceryNotice: 'Grocery item updated.' })
    );
    expect(togglePlanningGroceryStatePageServer).toHaveBeenCalledWith(state, 'grocery-1', {
      checked: true,
      excluded: false,
      onHand: false,
    });
    ...
    const addResponse = await POST({
      request: new Request('http://health.test/api/plan', {
        method: 'POST',
        body: JSON.stringify({
          action: 'addManualGrocery',
          state,
          draft: { label: 'Bananas', quantityText: '3' },
        }),
      }),
    } as Parameters<typeof POST>[0]);
    expect(await addResponse.json()).toEqual(
      expect.objectContaining({ groceryNotice: 'Manual grocery item added.' })
    );
    expect(addManualPlanningGroceryItemPageServer).toHaveBeenCalledWith(state, {
      label: 'Bananas',
      quantityText: '3',
    });
    ...
    const removeResponse = await POST({
      request: new Request('http://health.test/api/plan', {
        method: 'POST',
        body: JSON.stringify({ action: 'removeManualGrocery', state, itemId: 'grocery-1' }),
      }),
    } as Parameters<typeof POST>[0]);
    expect(await removeResponse.json()).toEqual(
      expect.objectContaining({ groceryNotice: 'Manual grocery item removed.' })
    );
