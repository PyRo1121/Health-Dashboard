...
describe('Sobriety route', () => {
	...
	it('logs sober status, craving, and lapse context', async () => {
		...
		await fireEvent.input(screen.getByLabelText('Craving note'), { target: { value: 'Stress spike after lunch.' } });
		await fireEvent.click(screen.getByRole('button', { name: 'Log craving' }));

		await fireEvent.input(screen.getByLabelText('Lapse note'), { target: { value: 'Had a lapse after a rough evening.' } });
		await fireEvent.input(screen.getByLabelText('Recovery action'), { target: { value: 'Text sponsor' } });
