...
describe('Nutrition route', () => {
	...
	it('saves a custom food and surfaces it in the local catalog', async () => {
		...
		await fireEvent.input(screen.getByLabelText('Meal name'), { target: { value: 'Turkey chili' } });
	...
	it('loads a recommended food directly into the meal draft', async () => {
		...
		await waitFor(() => {
			expect((screen.getByLabelText('Meal name') as HTMLInputElement).value).toBe('Greek yogurt bowl');
	...
	it('plans the current draft and can reload it later', async () => {
		...
		await screen.findByLabelText('Meal name');
		await fireEvent.input(screen.getByLabelText('Meal name'), { target: { value: 'Protein oats' } });
		...
		await waitFor(() => {
			...
			expect(screen.getByText('Protein oats')).toBeTruthy();
		});

		await fireEvent.input(screen.getByLabelText('Meal name'), { target: { value: 'Changed draft' } });
