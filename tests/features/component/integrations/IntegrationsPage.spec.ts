...
describe('Integrations route', () => {
	...
	it('shows the HealthKit companion manifest and import handoff', async () => {
		...
		await waitFor(() => {
			...
			expect(screen.getByRole('link', { name: 'Open import center' }).getAttribute('href')).toBe('/imports');
			expect(screen.getByRole('table', { name: 'Clinical connector capability matrix' })).toBeTruthy();
