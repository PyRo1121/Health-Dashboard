...
describe('Today route', () => {
	...
	it('saves a daily check-in and shows the event stream', async () => {
		...
		await fireEvent.input(screen.getByLabelText('Today note'), { target: { value: 'Steady start.' } });
	...
	it('shows explicit stale-item messaging when planned handoffs no longer resolve', async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText('That planned meal no longer exists. Replace it in Plan before logging it today.')
			).toBeTruthy();
			expect(
				screen.getByText('That planned workout no longer exists. Replace it in Plan before using it today.')
			).toBeTruthy();
		});
	});
});
