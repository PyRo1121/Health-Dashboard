...
describe('Health route', () => {
	...
	it('logs symptom, anxiety, sleep context, and a reusable supplement template', async () => {
		...
		await fireEvent.input(screen.getByLabelText('Symptom note'), { target: { value: 'After lunch' } });
		...
		await fireEvent.input(screen.getByLabelText('Anxiety trigger'), { target: { value: 'Crowded store' } });
		await fireEvent.click(screen.getByRole('button', { name: 'Log anxiety' }));

		await fireEvent.input(screen.getByLabelText('Sleep note'), { target: { value: 'Woke up twice' } });
		...
		await fireEvent.input(screen.getByLabelText('Template name'), { target: { value: 'Magnesium glycinate' } });
		...
		await waitFor(() => {
			const healthStream = screen.getByRole('heading', { name: 'Today’s health stream' }).closest('section');
			...
			expect(within(healthStream as HTMLElement).getByText('Anxiety episode', { exact: true })).toBeTruthy();
			expect(within(healthStream as HTMLElement).getByText('Sleep note', { exact: true })).toBeTruthy();
