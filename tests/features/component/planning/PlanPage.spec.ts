...
describe('Plan route', () => {
	...
	it('creates a workout template and uses it in the weekly board', async () => {
		...
		await waitFor(() => {
			...
			expect(screen.getByText(/Recovery · 2 exercises · Goblet squat · 3x8 · 60s rest/i)).toBeTruthy();
		});
	...
	it('shows a visible error when a selected recipe disappears before save', async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText('That recipe no longer exists. Choose another before adding it to the week.')
			).toBeTruthy();
		});
